...
int main() {
  ...
  .CHECK_OK (treeTest.insert(6)) //Checks if inserts 6.
  ...
  .print()
  .CHECK_EQUAL (treeTest.parent(6), 7) //Checks if 7 is the parent of 6.
  .CHECK_EQUAL (treeTest.left(7), 6) //Checks if 6 is the left son of 7.
  ...
  .CHECK_EQUAL (treeTest.contains(10), false)
  .CHECK_EQUAL (treeTest.root(), 12) //Checks if the root of the tree is 12.
  .CHECK_EQUAL (treeTest.size() ,4) //Checks if the size of the tree is 4.
  .CHECK_THROWS (treeTest.parent(12)) //Checks if throw exeption because 12 is the root so he dosn't have a parent.
